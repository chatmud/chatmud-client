...
export const useChannelHistory = (client: MudClient | null) => {
  ...
  const bufferedMessagesQueue = useRef<Message[]>([]);
  const isReplayingBuffer = useRef(false);
  ...
  // Flush buffered messages queue to state
  const flushBufferedMessages = () => {
    if (bufferedMessagesQueue.current.length === 0) return;

    setBuffers(prevBuffers => {
      const newBuffers = new Map(prevBuffers);
      const allBuffer = newBuffers.get("all");

      if (!allBuffer) return prevBuffers;

      const updatedMessages = [...allBuffer.messages, ...bufferedMessagesQueue.current];

      // Trim if exceeds max
      const trimmedMessages = updatedMessages.slice(-MAX_MESSAGES_PER_BUFFER);
    ...
    console.log(`[ChannelHistory] Flushed ${bufferedMessagesQueue.current.length} buffered messages`);
    bufferedMessagesQueue.current = [];
  };
  ...
  const handleMessage = (message: string, isBuffered: boolean = false) => {
    ...
    if (isReplayingBuffer.current && isBuffered) {
      ...
      // Add live messages immediately
      addMessageToBuffer("all", message);
    }
  };

  // Handle buffer replay events
  const handleBufferReplayStart = (count: number) => {
    console.log(`[ChannelHistory] Buffer replay starting: ${count} messages`);
    isReplayingBuffer.current = true;
    bufferedMessagesQueue.current = [];
  };

  const handleBufferReplayComplete = (count: number) => {
    console.log(`[ChannelHistory] Buffer replay complete: ${count} messages`);
    flushBufferedMessages();
    isReplayingBuffer.current = false;
  };

  // Handle channel messages
  const handleChannelText = (data: { channel: string; talker: string; text: string }) => {
    const { channel, talker, text } = data;
    const fullMessage = `[${channel}] ${text}`;
    ...
    if (!buffers.has(channel)) {
      ...
      setTimeout(() => addMessageToBuffer(channel, fullMessage, channel, talker), 0);
    } else {
      addMessageToBuffer(channel, fullMessage, channel, talker);
    }
  };
  ...
  useEffect(() => {
    ...
    client.on("bufferReplayStart", handleBufferReplayStart);
    client.on("bufferReplayComplete", handleBufferReplayComplete);

    return () => {
      client.removeListener("channelText", handleChannelText);
      client.removeListener("message", handleMessage);
      client.removeListener("bufferReplayStart", handleBufferReplayStart);
      client.removeListener("bufferReplayComplete", handleBufferReplayComplete);
    };
  }, [client, buffers]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Platform detection (used for multiple shortcuts below)
      const isMac = navigator.platform.toUpperCase().indexOf('MAC') >= 0;

      // Alt+Left/Right (Windows/Linux) or Command+Left/Right (Mac) - change buffers
      const isBufferSwitchModifier = isMac ? e.metaKey : e.altKey;

      if (e.key === "ArrowLeft" && isBufferSwitchModifier && !e.ctrlKey && !e.shiftKey) {
        e.preventDefault();
        changeBuffer(-1);
        return;
      }

      if (e.key === "ArrowRight" && isBufferSwitchModifier && !e.ctrlKey && !e.shiftKey) {
        e.preventDefault();
        changeBuffer(1);
        return;
      }

      // Command+1-0 (Mac) / Alt+1-0 (Windows): Read from current buffer
      const isCurrentBufferModifier = isMac
        ? (e.metaKey && !e.ctrlKey && !e.altKey)
        : (e.altKey && !e.ctrlKey && !e.metaKey);

      if (isCurrentBufferModifier && !e.shiftKey) {
      ...
      // Alt+Up/Down: Navigate within buffer
